...
public class TextLayerCollector extends KeyPathAttributeCollector<SolidLayer> {

    public TextLayerCollector() {
        super(SolidLayer.class);
    }
    ...
    public List<ViewAttribute> collect(
        ...
        attributes.add(new MutablePropertyFloatViewAttribute("TEXT_TRACKING", 0f) {
            @Override
            protected void mutate(Float value) {
                view.addValueCallback(
                        keyPath,
                        LottieProperty.TEXT_TRACKING,
                        new LottieRelativeFloatValueCallback(value)
                );
                view.resumeAnimation();
            }
